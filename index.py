from flask import Flask,flash,request,render_template,Response,redirect,session,url_for,g
import time
...
def check():
        ...
        if len(emailCorrectCount)==1:
            ...
            return redirect(url_for('yourdevice'))
        # return render_template("yourdevice.html")
        else:
            connec.close()
            flash("Please check user name or password")
            return render_template("new_login.html")
...
def insert():
    ...
    if not(len(password)==0) and (password==repassword):
        if len(emailCount) == 0:
            ...
            connec.commit()
            flash("Sucessfully registered go to login")
            Login="OK"
            connec.close()
            return render_template("new_login.html")
        else:
...
def setschedule():
    ...
    connec.commit()
    connec.close()
    flash("Schedule had been updated Start time: "+str(start)+"End time: "+str(end))
    return redirect(url_for('yourdevice'))
...
def setonoff():
    ...
    if data[0][1] == 0:
        ...
        connec.commit()
        onoffdetails="Your Device is ON Now"
    else:
        curso.execute("Update onoff set onoff=(%s) Where id = 1",0)
        connec.commit()
        onoffdetails="Your Device is OFF Now"
    return onoffdetails
...
def upload_pc_data():
    ...
    analog=request.args.get('analog')
    ...
    presentdate=date.today()
    curso.execute("insert into consumption(date,time,power,current,irms,analog) values (%s,%s,%s,%s,%s,%s)",(presentdate,presenttime,power,current,irms,analog))
    connec.commit()
    connec.close()
    return "successfully uploaded"
...
def contactus():
    ...
    connec.commit()
    connec.close()
    flash("successfully sent We will reach you as soon as possible")
    return render_template("home.html")
...
def yourdevice():
    ...
    connec.close()
    if g.user:
        return render_template( 'new_yourdevice.html', g1=g1,g2=g2,powerandcurrentdata=powerandcurrentdata,date=date,onoffdetails=onoffdetails,deviceSchedule=deviceSchedule,workingtime=workingtime)
    else:
...
#     #     connec.close()
#     #     onoffdetails="Your Device is ON Now"
#     #     return render_template("new_yourdevice.html",onoffdetails=onoffdetails)
#     # else:
#     #     curso.execute("Update onoff set onoff=(%s) Where id = 1",0)
#     #     connec.commit()
#     #     connec.close()
...
def testing():
    irms=request.args.get('irms')
    analog=request.args.get('analog')
    analog2=request.args.get('analog2')
    # power=float(powerandcurrent[:-4])
    # current=float(powerandcurrent[:4])
    ...
    curso.execute("insert into testing(irms,analog,analog2) values (%s,%s,%s)",(irms,analog,analog2))
    connec.commit()
    connec.close()
    return "successfully uploaded"
